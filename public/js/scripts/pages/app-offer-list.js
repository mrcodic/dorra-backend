...
// When user confirms in the modal, submit the hidden form
    ...
    $(document).on("submit", "#bulk-delete-form", function (e) {
        ...
        /*
         * Category changed
         * Category(parent) -> Products(children)
         */
        $('#addCategorySelect').on('change', function () {

        const categoryIds = $(this).val() || [];
        const $products = $('#addCategoryProductsSelect');
        ...
        $.ajax({
        url: "{{ route('offers.products-by-categories') }}",
        ...
        const selected =
        oldSelected.includes(productId);
        ...
        const type = $('input[name="product_scope"]:checked').val();

        if (type === 'with_category') {
        ...
        $('#addCategoryProductsSelect')
        .prop('disabled', false);

        $('#addProductsWithoutCategorySelect')
        .prop('disabled', true)
        .val(null)
        .trigger('change');
