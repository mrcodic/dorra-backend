...
@section('content')
    ...
    <div class="bg-white p-2">
        ...
        <form action="{{ route("settings.notifications.update") }}" method="POST" id="settingNotificationForm">
            @csrf
                            ...
                            <th>
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label"
                                           for="{{ $scopeId }}">{{ Str::headline($scope) }}</label>
                                </div>
                            </th>
                            <th>Email</th>
                            <th>Notification</th>
...
@section('vendor-script')
    <script>
        handleAjaxFormSubmit("#settingNotificationForm", {
        ...
        (function () {
            document.querySelectorAll('.select-all').forEach(function (master) {
                master.addEventListener('change', function () {
                    ...
                    const tbody = document.querySelector('tbody[data-scope="' + scope + '"]');
                    if (!tbody) return;
                    tbody.querySelectorAll('input.permission-checkbox, input.row-checkbox').forEach(cb => {
                        cb.checked = master.checked;
                    });
                });
            });

            document.querySelectorAll('tbody .row-checkbox').forEach(function (rowCb) {
                rowCb.addEventListener('change', function () {
                    const tr = this.closest('tr');
                    tr.querySelectorAll('input.permission-checkbox').forEach(cb => cb.checked = rowCb.checked);
                });
            });
        })();
    </script>
