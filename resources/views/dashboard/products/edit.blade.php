  ...
  <span class="image-hint small text-end">
    ...
    <script>
        function resetDropzoneElement(element, message) {
            ...
            // Some old rows keep the hidden input inside the dropzone element.
            // Preserve hidden inputs before clearing Dropzone preview DOM.
        ...
        function resetClonedOptionValueRow($row) {
            // jQuery repeater clones the first option row.
            // New value must not keep ids/images/dropzone previews from the cloned row.
            ...
            // Frame image reset.
            if (typeof window.ensureOptionFrameWrappers === 'function') {
                window.ensureOptionFrameWrappers($row[0]);
            }
            ...
            $outerRepeater.repeater({
                ...
                repeaters: [{
                    ...
                    show: function () {
                        ...
                        // ✅ IMPORTANT: repeater clones the first value row.
                        // Clear cloned ids/images/dropzone previews before initializing Dropzone.
                        if (typeof resetClonedOptionValueRow === 'function') {
                        ...
                        const dzElement = $row.find(".option-dropzone")[0];
                        ...
                        setTimeout(function () {
                            if (typeof window.refreshTableauFrameUploaders === 'function') {
                                window.refreshTableauFrameUploaders($row[0]);
                            } else if (window.selectedProductIsTableau && typeof initOptionFrameDropzones === 'function') {
                                initOptionFrameDropzones($row[0]);
                            }
    ...
    <script !src="">
        ...
        function initOptionDropzone(element, existingMedia = null) {
            const optionItem = element.closest("[data-repeater-item]");
            let uploadedInput = optionItem ? optionItem.querySelector(".uploadedImage") : null;

            // Safety: if an old cloned Dropzone lost the hidden input, recreate it.
            if (!uploadedInput && optionItem) {
                uploadedInput = document.createElement('input');
                uploadedInput.type = 'hidden';
                uploadedInput.name = 'option_image';
                uploadedInput.className = 'uploadedImage';
                element.insertAdjacentElement('afterend', uploadedInput);
            }
            ...
            let dz = new Dropzone(element, {
                ...
                init: function () {
                    ...
                    // ✅ Show existing image only for real edit rows.
                    // New cloned rows have an empty hidden input after reset.
                    if (existingMedia && uploadedInput && uploadedInput.value) {
                        let mockFile = {
                            name: existingMedia.file_name,
                            size: existingMedia.size,
                            _hiddenInputId: existingMedia.id
                        ...
                        uploadedInput.value = existingMedia.id;
                    }

                    // ✅ On success
                    dropzone.on("success", function (file, response) {
                        if (response?.data?.id && uploadedInput) {
                            file._hiddenInputId = response.data.id;
                            uploadedInput.value = response.data.id;
                        }
                    });

                    // ✅ On remove
                    dropzone.on("removedfile", function (file) {
                        let hiddenInput = uploadedInput;
    ...
    <script>
        ...
        function initOptionFrameDropzone(element, existingMedia = null) {
            ...
            new Dropzone(element, {
                ...
                init: function () {
                    // Do not restore copied existing frame media for newly cloned option rows.
                    // Existing edit rows have a hidden value; new rows are reset to an empty hidden value.
                    if (existingMedia?.id && hiddenInput.value) {
        ...
        function initOptionFrameDropzones(context = document) {
            ...
            root.querySelectorAll(".option-frame-dropzone").forEach(function (element) {
                const wrapper = element.closest(".option-frame-wrapper");
                const hiddenInput = wrapper?.querySelector(".option-frame-image-hidden");
                const existingMedia = hiddenInput?.value
                    ? parseTableauMediaDataset(element.dataset.existingFrameMedia)
                    : null;
    ...
    <script>
        (function () {
            ...
            function optionItemsInside(context) {
                ...
                if ($context.is('[data-repeater-list="specification_options"] > [data-repeater-item]')) {
            ...
            window.ensureOptionFrameWrappers = function (context) {
                optionItemsInside(context).each(function () {
                    const $optionItem = $(this);

                    if ($optionItem.find('.option-frame-wrapper').length) return;

                    const $optionImageCol = $optionItem.find('.uploadedImage').last().closest('.col-md-12');

                    if ($optionImageCol.length) {
                        $optionImageCol.append(frameUploaderHtml());
                    }
                });
            };

            window.resetClonedOptionFrameUploader = function (context) {
                window.ensureOptionFrameWrappers(context);
