...
if (!function_exists('handleMediaUploads')) {
    function handleMediaUploads(
        ...
        $sampleColor = static function (
            ...
            $x0 = max(
                0,
                min($cx - intdiv($size, 2), max($w - 1, 0))
            );

            $y0 = max(
                0,
                min($cy - intdiv($size, 2), max($h - 1, 0))
            );
        ...
        $applyTransparency = static function ($file) use (
            ...
            try {
                ...
                /*
                 * Sample multiple points around the edges instead
                 * of removing every white pixel from the entire image.
                 */
                $stepX = max(
                    1,
                    (int) floor($width / 20)
                ...
                foreach ($points as [$x, $y]) {
                    ...
                    /*
                     * If transparentColor exists, remove that color.
                     * Otherwise detect background color from edge.
                     */
                    ...
                    /*
                     * floodFill only removes pixels connected
                     * to this edge point.
                     *
                     * White text/product areas isolated from the
                     * background remain untouched.
                     */
