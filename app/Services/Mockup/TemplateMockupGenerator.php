...
class TemplateMockupGenerator
{
    ...
    public function generate(Mockup $mockup, array $colors = [], bool $force = false): void
    {
        if (empty($colors)) {
    ...
    {
        $colors = $this->cleanColors($mockup->pre_fill_colors ?? []);
    ...
    public function handleUpdated(Mockup $mockup): void
    {
        ...
        if (!empty($removedHexes)) {
            $this->removeDeletedColors($mockup, $removedHexes);
        }
    ...
    public function removeDeletedColors(Mockup $mockup, array $removedHexes): void
    {
        ...
        $mockup->templates()
            ->lazyById(100, 'templates.id', 'id')
            ->each(function ($template) use ($mockup, $removedHexes) {
    ...
    protected function generateForNewColors(Mockup $mockup, array $addedHexes): void
    {
        ...
        $mockup->templates()
            ...
            ->each(function ($template) use ($mockup, $bulkJob, $addedHexes, $hexToOriginalColor, &$totalCount) {
                ...
                if ($isPivotPositionsEmpty) {
                    $positions = $this->getDefaultPositions($mockup, $template);
                } else {
    ...
    protected function getTemplateDesignDimensions(Template $template): array
    {
        ...
        $media = $template->getFirstMedia('templates');

        if (!$media) {
            $media = $template->getFirstMedia('templates-preview');
        }
