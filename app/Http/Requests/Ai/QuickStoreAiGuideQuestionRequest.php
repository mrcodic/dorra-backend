...
class QuickStoreAiGuideQuestionRequest extends FormRequest
{
    ...
    {
        $options = collect($this->input('options', []))->values()->map(function ($option) {
            ...
            if (empty($colors)) return $option;

            $prompt = 'Use this exact color palette: ' . implode(', ', $colors);
            data_set($option, 'label.en', trim((string) data_get($option, 'label.en')));
            data_set($option, 'label.ar', trim((string) data_get($option, 'label.ar')));
            data_set($option, 'prompt_value.en', trim((string) data_get($option, 'prompt_value.en')) ?: $prompt);
            data_set($option, 'prompt_value.ar', trim((string) data_get($option, 'prompt_value.ar')) ?: $prompt);
            data_set($option, 'ui_data.colors', $colors);

            return $option;
        })->all();

        $hasPalette = collect($options)->contains(fn($option) => !empty(data_get($option, 'ui_data.colors', [])));

        $this->merge([
            'type' => $hasPalette ? AiGuideQuestionTypeEnum::SINGLE_SELECT->value : $this->input('type'),
            'options' => $options,
        ]);
    }
